    ...
    <div class="p-0 gradient">
      ...
      <div class="p-4 bg-white">
        ...
        <div id="action-tabs" class="bg-blue-600 flex justify-around pt-2 mt-8">
          ...
          >
            <span :class="tab.icon"></span> <sup>{{ getCount(tab.name)}}</sup> <br />
      ...
      <div
        id="action-area"
        class=" bg-white text-red-900 p-4"
      >
        <div v-if="currentAction == 'ads'">
          <ul v-if="user.ads.length>0">
            <li 
            v-for="result in user.ads" :key="result.id"
            ...
            >
                ...
                <div class="relative w-3/4">
                    <h2 class=" text-black font-bold p-2">
                      <router-link :to="'/ads/'+result.id">{{result.title}}</router-link>
                    </h2> 
                    <p class="p-2 line-clamp-2 md:line-clamp-none">
                        {{result.description}}
                    </p>
                    <div class="bg-gray-50 px-2 py-1 flex justify-between w-full  ">
                      <button class="w-1/3 bg-blue-700 text-white">Edit</button>
                      <button class="w-1/3 bg-red-700 text-yellow-50 p-1">Delete</button>
