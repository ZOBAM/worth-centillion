...
const actions = {
    ...
    fetchData({commit}){        
        ...
        axios.get(process.env.VUE_APP_APIURL+"/ads"+urlPostfix+urlPara).then(function(response){
            console.log(response.data);
        ...
        function setState(data){
            if(state[data] == null || totalAdsCount != localStorage.getItem('lastTotalAdsCount')){
                console.log(`${data} at the moment is null or doesn't match`);
                let localData = localStorage.getItem(data);
                if (localData && JSON.parse(localData) && totalAdsCount == localStorage.getItem('lastTotalAdsCount')) {
                    ...
                    axios.get(process.env.VUE_APP_APIURL+"/ads/get_"+data).then(function(response){
                        commit('setStateProps',{name: data, value: response.data[data]});
